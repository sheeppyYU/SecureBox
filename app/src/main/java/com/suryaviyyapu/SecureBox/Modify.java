...
public class Modify extends AppCompatActivity implements View.OnClickListener {
    ...
    public void copy_email(View view) {
        if (sharedPreferences.getBoolean(PREF_KEY_SECURE_CORE_MODE, false)) {
            Toast.makeText(this, "安全模式已打開，不允許用戶複製!", Toast.LENGTH_SHORT).show();
        } else {
            ...
            if (clipboard != null) {
                ...
                Toast.makeText(getApplicationContext(), "帳號複製!", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void copy_password(View view) {
        if (sharedPreferences.getBoolean(PREF_KEY_SECURE_CORE_MODE, false)) {
            ...
            if (clipboard != null) {
                clipboard.setPrimaryClip(clip);
                Toast.makeText(getApplicationContext(), "密碼複製!", Toast.LENGTH_SHORT).show();
            }
        }
    }
}